...
x(3)=(1-omega)/alfa;
disp(['Il valore di k di prima iterazione è: ', num2str(x(3))]);
...
%Atterraggio----------------------------------------------------
x(2)=(X_LA/1.66)*a_frenata*((rhosl*Cl_land)/omega);
disp(['Il valore di QM/S di prima iterazione è: ',num2str(x(2))]);
%Si scrive T0/S:

%Decollo---------------------------------------------------------------
x(4)=(x(2)^2)*(1/g)*1.75*(1/(XFR*Cl_toff*X_TO*rhosl));
disp(['Il valore di T0/S di prima iterazione è: ', num2str(x(4))]);

%Cruise--------------------------------------------------------------
x(4)=(1/(psi*zeta))*(0.5*rho*(V_cruise^2)*CD0_velivolo+((x(1)/Sw_1)^2)./(0.5*rho*(V_cruise^2)*e*pi*x(5)));
disp(['Il valore di T/S di prima iterazione è: ',num2str(x(4))]);

%Equazione stima pesi------------------------------------------------

N=n*1.5;   %Sovrastima del fattore di carico? 
L_fus=(n_pax/n_paxrow)*1.2*1.1;  %lunghezza fusoliera [m]
Q_ala=(1/7760)*N*(x(1)^(3/2))*(x(5)^(3/2))*sqrt(1/x(2))+6*(1/x(2))*x(1);
Q_fus=(1/1000)*N*x(1)*L_fus;
Q_impennaggi=3.5*(1/x(2))*x(1);
Q_carrello=0.03*x(1);
Q_impianti=0.2*x(1);
Q_motore=0.2*x(4)*(1/x(2))*x(1);
