...
CD0_velivolo=((2*cf_wing*f_spess*f_pres*1.2)+Cf_fuso*(S_fuso/((1/x(2))*x(1))))*f_int;


%EFFICIENZA
CL=(2*x(1))/(p.rho*(p.V_cruise^2)*(1/x(2))*x(1));
E=CL/(CD0_velivolo+(CL^2/(pi*p.e*x(5))));
%autonomia------------------------------
sys(1)=(E/p.c_s)*p.V_cruise*log(1/(1-p.alfa*x(3)));
%Atterraggio----------------------------------------------------
...
%Cruise--------------------------------------------------------------
sys(4)=(1/(p.psi*p.zeta))*(0.5*p.rho*(p.V_cruise^2)*CD0_velivolo+((x(1)/(x(1)/x(2)))^2)./(0.5*p.rho*(p.V_cruise^2)*p.e*pi*x(5)))-x(4);
...
Q_f=x(3)*x(1);
sys(5)=Q_ala+Q_fus+Q_impennaggi+Q_carrello+Q_impianti+Q_motore+Q_fisso+Q_f-x(1);
end