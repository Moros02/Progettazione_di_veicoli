...
%%%% PROVA %%%%
mdl_wf=fitlm(Database.W_MTO, Database.W_f, 'linear', 'VarNames',{'W_MTO','W_f'});
x=linspace(0,100000,300);
y=predict(mdl_wf,x');
figure
scatter(Database.W_MTO, Database.W_f,'filled')
hold on
plot(x,y,'-r','LineWidth',1);
xlabel('W_MTO');
ylabel('W_f');
...
mdl_wp=fitlm(Database.W_MTO, Database.W_p, 'linear', 'VarNames',{'W_MTO','W_p'});
x=linspace(0,100000,300);
y=predict(mdl_wp,x');
figure
scatter(Database.W_MTO, Database.W_p,'filled')
hold on
plot(x,y,'-r','LineWidth',1);
xlabel('W_MTO');
ylabel('W_p');
